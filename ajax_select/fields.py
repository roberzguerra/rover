...
# def _check_can_change(self, user, related_model):
#     """
#     Verifica se o usuario possui permissao para Alterar o conteudo do Model utilizado.
#
#     Se o LookupChannel implmenetar check_can_change() entao usara este metodo.
#
#     Se nao usa o sistema padrao de permissoes do Django.
#
#     Se permitir a alteracao do model, habilita o widget para exibir o link de edicao.
#     """
#     lookup = registry.get(self.channel)
#     if hasattr(lookup, 'can_change'):
#         can_add = lookup.can_add(user, related_model)
#     else:
#         ctype = ContentType.objects.get_for_model(related_model)
#         can_add = user.has_perm("%s.change_%s" % (ctype.app_label, ctype.model))
#     if can_add:
#         self.widget.add_link = reverse('change_popup', kwargs={
#             'app_label': related_model._meta.app_label,
#             'model': related_model
#         })
